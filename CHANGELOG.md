...
- **HAR export.** Logs → **HAR** downloads a Pendo-only network capture. Chrome and Edge reload the tab once behind a two-click confirm and record via CDP in the service worker, so the capture survives the reload; the panel reopens on the tab it was captured from. Firefox falls back to Resource Timing without reloading. Cookie, `Authorization`, and `Set-Cookie` headers are stripped, and the panel's own self-instrumentation requests are excluded.
...
- **Tests.** 638 tests across 31 suites.
