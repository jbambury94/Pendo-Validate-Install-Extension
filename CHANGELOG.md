...
- **Logs tab.** **Copy visible** and **Download** now honour the level chips and search box; filter and query persist across sessions (`logUiPrefs`). Checks with matching console output link straight to the filtered Logs view. **Install details** consolidates identity, metadata, and install snapshot fields (formerly split across Status and Settings). **HAR** downloads a Pendo-only network capture (Chrome/Edge: CDP + reload with two-click confirm; Firefox: Resource Timing fallback) — gated, see below.
- **Feature gates for work that isn't fully baked.** Three features now ship switched off and have to be opted into per install: **HAR download** (`harDownload`), **AI remediation advice** (`aiAdvice`), and **CSP probing** (`cspProbe`, registered ahead of the implementation so it cannot reach users by accident). With a gate closed the UI is absent *and* the underlying capability is unreachable — the service worker refuses to attach the debugger for a HAR capture or to proxy a provider request, so hiding a button is never the only thing standing between a half-built feature and a customer. Shipped defaults live in `extension/feature-flags.json`; per-install overrides live in `chrome.storage.local` under `featureOverrides` and are written from the panel console with `__pendoValidateFeatures.enable('harDownload')`. Everything fails closed: an unreadable registry, a malformed override, an unknown key, or a gate whose browser lacks the required API all leave the feature off. A saved AI API key is left untouched while the AI gate is closed, so re-enabling restores your setup. Flipping a gate emits a `feature_flag_toggled` Track Event, and `validation_completed` now reports gate state.
...
- **Tests.** 708 tests across 33 suites, including a gating suite that runs against the shipped `popup.js` rather than the `tests/helpers.js` mirror, and a panel suite that boots the real `popup.html` to confirm a closed gate leaves the UI absent.
