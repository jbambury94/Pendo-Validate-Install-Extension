...
- **Deeper checks on every Validate.** **Validate Pendo Install** now runs five extra checks alongside `validateInstall()`. There is no separate diagnostics mode. The checks always call the page's own agent, never the panel's self-instrumentation instance, and a static test enforces that.
  ...
  - **Duplicate installs and multiple API keys:** flags an agent script included more than once, and agents for more than one API key on the same page.
  - **Anonymous visitors:** a temporary `_PENDO_T_` visitor ID is flagged as a warning and grades the visitor ID as poor. Previously it passed as a real visitor ID.
  - **Frame map:** a probe runs in every frame (`allFrames`) and records where Pendo is running, with which key and identity, and whether it is sending data. A new **Frames** card appears when the page has subframes. It flags subframes that use a different key or visitor from the top page. When Pendo runs only in a subframe, the hero says **Pendo found in a subframe** instead of "Pendo not found", and the "not detected" and "No API key detected" advice is dropped.
  - **Network capture (Chrome and Edge, opt-in):** a new Settings → **Network capture** toggle, off by default, makes Validate reload the page and record Pendo requests with CDP in the service worker. The panel reopens and finishes validating when the capture completes. A new **Network** card groups requests by kind (agent script, event data, guides, and so on) and reports failures as blocked by CSP, blocked by the client (ad blockers), HTTP errors, or network errors. It also shows the document's CSP response header. A blocked agent script is an error; other failures are warnings. Logs → **HAR** reuses the capture without a second reload. Firefox hides the toggle.
- **Hero counts the new checks.** Findings from the new checks carry an explicit severity. The Status hero, the Markdown and Share status lines, and the `validation_completed` outcome now count them, so a clean console no longer reads "All checks passed" next to a blocked agent script. Older advice is counted as before.
- **Install details** adds an **Anonymous visitor** row and an **Environment** section: **Config options** (non-default count and list), agent errors (newest first), plugins, the agent-script count, and API keys seen. The generic "Environment check: Available" row is shown only when the check is unavailable or failed.
- **Reports.** The Markdown report adds environment, agent-script, API-key, anonymous, frame, and network fields to its Metadata JSON, plus **Agent environment**, **Frames**, and **Network** sections. The JSON export leaves out the raw HAR. With identity off, Share redacts subframe IDs, keys, and frame hosts.
- **Telemetry.** `validation_completed` gains `ivaFrames`, `ivaSubframe`, `ivaAgentErrs`, `ivaDup`, `ivaAnonymous`, and `ivaNetFails`. All are bucketed or boolean, and none carries URLs, IDs, keys, or console text. `PRIVACY.md` documents each property, the new storage keys (`networkCaptureOnValidate`, `pendingNetworkCapture`), and what the network capture keeps on the device.
- **Injection.** `capture-inspect.js` is now revision 4. New `frame-probe.js` (MAIN world, all frames) and `diagnostics.js` (panel-side analysis) are listed in `web_accessible_resources`. Validation still completes if the frame probe fails, and frames it can't reach (sandboxed, restricted, or still loading) are counted as not inspectable.
- **Tests.** 763 tests across 34 suites.

## 1.9.0
- **Share replaces the Export menu.** The action bar's Export flyout is gone; **Share** copies the plain-text summary in one click. Page URL and visitor/account IDs are omitted by default — enable **Include identity & page URL in Share** in Settings → Sharing to add them. With the opt-in off, known visitor/account/parent IDs, page URLs, and API-key-like strings are redacted from the passing checks and recommendations too, not just dropped from the identity lines.
...
- **Logs tab.** **Copy visible** and **Download** now honour the level chips and search box; filter and query persist across sessions (`logUiPrefs`). An err/warn check whose text appears in the captured console offers **View in Logs**, which seeds the search, narrows the chips to that level, and switches tabs — the link is shown only when it will land on matching lines. **Install details** consolidates identity, metadata, and install snapshot fields (formerly split across Status and Settings).
- **HAR export.** Logs → **HAR** downloads a Pendo-only network capture. Chrome and Edge reload the tab once behind a two-click confirm and record via CDP in the service worker, so the capture survives the reload; the panel reopens on the tab it was captured from, and entries carry per-phase timings (blocked, DNS, connect, SSL, send, wait, receive) from CDP. If the debugger can't attach — for example DevTools is already open on the tab — the panel stays open and shows the error instead of reloading. Firefox falls back to Resource Timing without reloading. Requests are included by parsed hostname (`*.pendo.io` and Pendo's Cloud Storage buckets) or, on custom CNAME domains, only Web SDK agent/data routes that carry an API key. Cookie, `Authorization`, and `Set-Cookie` headers are stripped, and the panel's own self-instrumentation requests are excluded.
- **Self-instrumentation Track Events.** The panel fires `validation_completed`, `share_summary_copied`, `markdown_report_downloaded`, `debugger_enabled`, and `har_downloaded` via client-side `pendo.track()`. No event carries the validated page's URL, its visitor/account/parent IDs, the API key, or console text; log volume is bucketed to keep cardinality low. `PRIVACY.md` documents each event property by property.
...
- **Firefox packaging.** Event pages have no `importScripts`, so the transformed manifest loads `har-capture.js` ahead of the background script.
- **Tests.** 655 tests across 31 suites.
