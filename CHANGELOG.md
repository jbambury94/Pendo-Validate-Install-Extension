...
- **Tests.** New `tests/extBridge.test.js` covers the browser-first messaging bridge (Firefox dual-namespace resolution, `chrome` callback promisification, `lastError` rejection), and `tests/injectedScripts.test.js` gains host-page global-collision cases; the bridge helpers are mirrored into `tests/helpers.js` and the manifest version assertion bumped to 1.8.4. 520 tests across 23 suites.
