...
- **Tests.** New redirect/URL-sanitization and client-side URL-sanitization cases in `tests/captureAndInspect.test.js`, a `vds` case in `tests/inferSupportKeyFromText.test.js`, KB coverage in `tests/pendoKb.test.js`, and the manifest version assertion bumped to 1.8.2; helpers synced in `tests/helpers.js`. 477 tests across 21 suites.
